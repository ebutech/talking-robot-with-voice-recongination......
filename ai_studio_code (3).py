...
# (Updated for potential Speaker Differentiation)
#
...
def calculate_rms(frame):
    """Calculate the Root Mean Square (RMS) of a given audio frame."""
    count = len(frame) // 2
    format_str = f"<{count}h"  # 'h' for short (2 bytes), '<' for little-endian
    try:
        shorts = struct.unpack(format_str, frame)
    except struct.error:
        # Handle cases where frame length might not be an even number of shorts
        return 0.0
    sum_squares = sum(s**2 for s in shorts)
    return math.sqrt(sum_squares / count) if count > 0 else 0.0

def calibrate_noise_level(stream):
    """Calibrate the background noise level to set a dynamic threshold."""
    print(f"[System]: Calibrating background noise for {CALIBRATION_SECONDS} seconds. Please be quiet.")
    rms_values = []
    num_frames = int(CALIBRATION_SECONDS * 1000 / VAD_FRAME_MS)
    for _ in range(num_frames):
        try:
            frame = stream.read(VAD_CHUNK_SIZE, exception_on_overflow=False)
            rms = calculate_rms(frame)
            if rms > 0: # Only consider frames with actual sound
                rms_values.append(rms)
        except IOError as e:
            print(f"[Warning]: IOError during calibration: {e}")
            if len(rms_values) < num_frames / 2:
                pass
    
    if not rms_values:
        print("[Warning]: Could not collect sufficient audio data for calibration. Using a default threshold.")
        return 50 # Default fallback threshold
    
    average_rms = sum(rms_values) / len(rms_values)
    noise_threshold = average_rms * NOISE_THRESHOLD_MULTIPLIER
...
def pcm_to_wav(frames, sample_width):
    """Converts a list of PCM audio frames to WAV format."""
    buffer = io.BytesIO()
...
# --- Async Audio Handling Functions ---
async def send_audio(ws, stop_event, stream, audio_instance, initial_noise_threshold, is_playing_event):
    """
    Captures audio from the microphone, performs Voice Activity Detection (VAD),
    and sends detected speech to the WebSocket server.
    """
    loop = asyncio.get_running_loop()
    vad = webrtcvad.Vad(VAD_AGGRESSIVENESS)
    
    padding_frames = deque(maxlen=PADDING_FRAMES_COUNT)
    silence_frames_needed = int(SILENCE_DURATION_S * 1000 / VAD_FRAME_MS)
    max_frames = int(MAX_RECORDING_S * 1000 / VAD_FRAME_MS)
    sample_width = audio_instance.get_sample_size(FORMAT)

    noise_threshold = initial_noise_threshold
    recalibration_buffer = deque()
    recalibration_bytes_trigger = int((RATE * sample_width) * (RECALIBRATION_AUDIO_MS / 1000))
    
    voiced_confirmation_frames = 0
    ...
    try:
        while not stop_event.is_set():
            if is_playing_event.is_set():
                ...
                padding_frames.clear()
                recalibration_buffer.clear()
                voiced_confirmation_frames = 0
            ...
            rms = calculate_rms(frame)
            is_speech_and_loud = vad.is_speech(frame, RATE) and rms > noise_threshold
            
            padding_frames.append(frame)

            if is_speech_and_loud:
                voiced_confirmation_frames += 1
            else:
                voiced_confirmation_frames = 0
                recalibration_buffer.append(frame)
                current_recal_size = sum(len(b) for b in recalibration_buffer)
                
                if current_recal_size > recalibration_bytes_trigger:
                    print(f"\r[VAD]: Performing auto-recalibration...{' '*40}", end="", flush=True)
                    pcm_data = b''.join(recalibration_buffer)
                    recalibration_buffer.clear()
                    
                    new_baseline_rms = calculate_rms(pcm_data)
                    old_avg_noise = noise_threshold / NOISE_THRESHOLD_MULTIPLIER
                    
                    new_avg_noise = (old_avg_noise * (1 - RECALIBRATION_SMOOTHING_FACTOR)) + \
                                    (new_baseline_rms * RECALIBRATION_SMOOTHING_FACTOR)
                    
                    noise_threshold = new_avg_noise * NOISE_THRESHOLD_MULTIPLIER
                    print(f"\r[VAD]: Auto-recalibration complete. New threshold: {noise_threshold:.2f}{' '*30}", end="", flush=True)
            ...
            if voiced_confirmation_frames >= VAD_TRIGGER_CONFIRMATION_FRAMES:
                ...
                voiced_frames = list(padding_frames)
                ...
                while not stop_event.is_set():
                    ...
                    current_rms = calculate_rms(frame)
                    if vad.is_speech(frame, RATE) and current_rms > noise_threshold:
                        silent_frames_count = 0
                    else:
                    ...
                    if silent_frames_count >= silence_frames_needed or len(voiced_frames) >= max_frames:
                        ...
                        wav_data = pcm_to_wav(voiced_frames, sample_width)
                        encoded_data = base64.b64encode(wav_data).decode('utf-8')
                        
                        # We could potentially add a 'speaker_id' here if the client could identify it,
                        # but typically this is done server-side.
...
async def receive_messages(ws, stop_event, is_playing_event):
    """
    Receives messages from the WebSocket server, processes transcripts,
    and plays back audio streams using ffplay.
    Updated to handle potential 'speaker_id' in transcript messages.
    """
    loop = asyncio.get_running_loop()
    ffplay_process = None
    try:
        async for message in ws:
            ...
            if msg_type == "transcript":
                text = msg.get('text', '')
                speaker_id = msg.get('speaker_id') # Look for speaker ID
                
                if speaker_id:
                    print(f"[Speaker {speaker_id} said]: {text}")
                else:
                    print(f"[You said]: {text}") # Fallback if no speaker ID
                ...
                if speaker_id:
                    print(f"\r[Speaker {speaker_id}]: {text}", end="", flush=True)
                else:
                    print(f"\r[Bot]: {text}", end="", flush=True) # Fallback if no speaker ID
            elif msg_type == 'audio-stream-start':
                is_playing_event.set()
                print(f"\r[Player]: Receiving audio stream...{' '*40}", end="", flush=True)
                ...
                if ffplay_process and ffplay_process.stdin and not ffplay_process.stdin.closed:
                    try:
                        ...
                        await loop.run_in_executor(None, ffplay_process.stdin.write, audio_chunk)
                    except (IOError, BrokenPipeError):
                        print("[Player]: Warning: Broken pipe to ffplay. Restarting player if needed.")
                        ffplay_process = None
                ...
                await asyncio.sleep(MIC_POST_PLAYBACK_COOLDOWN_S)
...
async def blink_led_task(is_playing_event, stop_event, gpio_manager):
    """
    Asynchronously blinks an LED while audio is playing.
    This task will only run if RPi.GPIO is successfully loaded.
    """
    if not gpio_manager.is_pi:
        return
    ...
    try:
        while not stop_event.is_set():
            ...
            while is_playing_event.is_set() and not stop_event.is_set():
                ...
                if not is_playing_event.is_set(): 
                    break 
...
async def main():
    """
    Main function to initialize resources, connect to WebSocket,
    and manage audio sending/receiving and LED blinking tasks.
    ...
    try:
        while True:
            ...
            is_playing_event = asyncio.Event()
            tasks = []
            ...
            if stop_event.is_set() and any(isinstance(task.exception(), (KeyboardInterrupt, asyncio.CancelledError)) for task in tasks if task.done()):
                break
...
if __name__ == "__main__":
    try:
        ...
        print("\n[System]: Main program received KeyboardInterrupt. Exiting.")